...
**************************************************************************/
  ...
  int k=0, i=0;
  int num = 11;  // number of Gaussian correction terms 
  double max[num], std[num], amp[num];
  ...
  double fac_b777_mod = 0.80416044;        // Skal-Faktor damit  Int_J(w)dw gleich fur J_B777 und J_B777_mod

  double jw_i[num], j_add;                // For B777-modified 
  ...
  else if(specdens == 2)  // Pieper spectral density fuer WSCP!! //Pieper 2011, JPCB, Artcile I, S.4042
  { 
  ...
  else if(specdens == 4)       // Renger-Marcus SpecDens modified with additional Gauss-funcs to fit Wendling-FMO-Specdens
  {
    printf("Spectral sensity from Renger and Marcus, modified to fit Wendling-FMO-SpecDen, Species=%2d\n", species);
    ...
    for(k=0; k < NT/2; k++)
    {
      ...
      for(i=0; i < num; i++)
      {
	jw_i[i] = amp[i]*exp( -SQ(w/WZ2WFS-max[i])/(2*SQ(std[i])) );
      }
