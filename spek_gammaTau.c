...
void gammaTau(int nz, double **rij, double *eigval, double **eigvec, 
  ...
  // Gibt doch eine relative grosse Abweichung fuer die Taus, wenn man cw_re[m] verwendet...
...
// ======================================================================================================0
  ...
  for(mm=0; mm<nz; mm++)
    for(kk=0; kk < nz; kk++)
    {
      w = (eigval[mm] - eigval[kk]);  // [cm-1]
      fprintf(out_file,"%3d %3d %12.6lf %10.6lf %10.6lf %12.6lf \n", 
	      mm, kk, w, gamma[mm][kk], tau[mm], tau_rec[mm]);
    }
  fclose(out_file);
  */
  ...
  printf("\n");
  for(kk=0; kk < nz; kk++)
    printf("gammTau.c  %3d   gamma = %lf,  tau= %8.1lf fs,   %6.2lf cm-1\n", 
	   kk, gamma[kk][kk], tau[kk], 1./tau[kk]/WZ2WFS);
  printf("\n");
  */
  ...
  for(k=0; k<nz; k++)  
    for(j=0; j<nz; j++) 
    {
      w = (nr_ew[k+1] - nr_ew[j+1])*WZ2WFS;  // [fs-1]

      m = NT/2 + round(w/WSTEP);

      printf("w, w/step, m, C_Re(w):  %10.6lf %6.1lf %6d %e\n", w, w/WSTEP, m, cw_re[m]);  // [cw]=cm-1
     
      cw_re_wmk[k][j] = cw_re[m]*WZ2WFS;   // [fs-1]
     }
 */
