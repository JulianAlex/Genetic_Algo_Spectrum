...
//===========================================================================
/*
int outputGauss(double *nr_ew, double *ngau_abs, int nz){

  FILE *out_file;
  char *out_dat="out_gauss.dat";
  int j=0;
  ...
  fprintf(out_file,"#  E(cm^-1)	   ABS         \n");
  for(j=0; j<NGAU; j++)
    fprintf( out_file,"%lf    %lf    \n",
	     SPEKMIN+(j+0.5)*GAULENG,  ngau_abs[j] );
  fclose(out_file);
