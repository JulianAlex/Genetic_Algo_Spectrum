...
// Macht einen komplizierten eindruck erst x,y,z einzulesen und dann in na,nb,nc,nd
//   umzubenennen, aber na,nb,... direkt einlesen ist auch kompliziert                 
...
void parameter(double *site, double *sigma, double *dipst, int bclnum, int nz){
  ...
  for(i=0; i<nz; i++){   // setze vorlaeufig alle Dipolstaerken gleich
    dipst[i] = DIPST;
  }
...
void matrix2dia(int nz, double *site, double *sigma, int *seed, double **vab,
		double *hamilt, int i)
{
...
//====================================================================================================
/*
void matrix2dia(int nz, double *site, double *sigma, double **vab,
		double **nr_ev, long *idum, int i)
{

  // WW-Matrix

  int j=0, k=0;
  double rand_nr=0.0, site_h[nz]; 

  for(j=0; j<nz; j++) site_h[j]=0.0; 

  if(i == 0)
    for(j=0; j<nz; j++){
      site_h[j] = site[j];  //im 1.Durchlauf wird nicht gezogen (wegen Gauß dressed Sticks Berechnung...)
    }
  else
    for(j=0; j<nz; j++){
      //rand_nr = gasdev(idum);   //Würfeln der Site Energies
      site_h[j] = site[j] + sigma[j]*rand_nr;
    }
