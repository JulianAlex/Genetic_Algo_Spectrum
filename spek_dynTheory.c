...
void dynTheoryArray(int nz, int anz, float ***sideband, float ***zeroline,
  ...
  for(j=0; j<nz; j++)  //init.
  { 
    spek_ab[j]=0; spek_cd[j]=0; spek_ld[j]=0; 
    ib[j]=0; ie[j]=0;
    msel[j]=0; nsel[j]=0;
 }
  ...
  while(nrow--)
  {
    sinexct_ab[nrow] = calloc( (anz+NT/2), sizeof(double) ); //+5
  ...
  {
    ib[m] = (int) ((SPEKMIN - w_m0_sh[m])/wstep - 0.5);
  ...
  for(m=0; m<nz; m++)
  {
    ...
    for(k=ib[m]; k<ie[m]; k++)
    {
      ...
      sinexct_ab[m][k+NT/2] = 4*PISQ*ww*NINDEX*alpha[m]/(3*HBARC*EVWZ)*lineshape*ALPHFAC; // [e^2 Ang s]
      sinexct_sb[m][k+NT/2] = 4*PISQ*ww*NINDEX*alpha[m]/(3*HBARC*EVWZ)*linesh_sb*ALPHFAC; 
      sinexct_zl[m][k+NT/2] = 4*PISQ*ww*NINDEX*alpha[m]/(3*HBARC*EVWZ)*linesh_zl*ALPHFAC; 

      sinexct_cd[m][k+NT/2] = NINDEX*ww/CVAK*rotst[m]*lineshape; 
...
void dynTheoryArrayHB(int nz, int anz, float ***sideband, float ***zeroline,
  ...
  for(m=0; m<nz; m++)  // Index Verschiebereien
  {
    ib[m] = (int) (SPEKMIN - w_m0_sh[m])/wstep;
    ie[m] = (int) (SPEKMAX - w_m0_sh[m])/wstep;
